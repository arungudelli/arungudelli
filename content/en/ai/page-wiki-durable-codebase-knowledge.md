...
LLMs don't get bored. They don't forget to update a cross-reference. They'll read fifteen files carefully, write down what they found, and do it again tomorrow without sighing once.
...
And that's what clicks into place. A knowledge wiki written and maintained by an LLM isn't a doc *for the machine* or a doc *for the human* — it's the same file serving both.

That's the win-win. **The human finally gets documentation that stays current. The LLM gets a ready-made map so it stops rediscovering everything from scratch.**

One artifact, two beneficiaries — and the party who'd normally never keep it updated (the **human**) is the one party that doesn't mind the chore (the **LLM**).

And look at what each side actually gets.

The **human** gets the thing our industry has wanted forever and never sustained: a living map of the codebase you can *read.* Open the wiki, understand how a page works, onboard in an afternoon — the knowledge wiki we could never keep alive, finally kept alive.

The **LLM** gets the same pages as a context primer. It reads the relevant page *first* and walks in already oriented — the right files, the data flow, the gotchas — instead of grepping around and burning tokens before it can even start.
...
Ten minutes later you ask for a related change to the **same** page — and it opens the same five files and reads them all over again, from scratch.
...
## 🗺️ What actually happens when you ask
...
flowchart LR
    subgraph without [Without a wiki - grep the jumbled dictionary]
        direction TB
        A1([Prompt]) --> A2[grep / glob the repo]
        A2 --> A3[Open a candidate file]
        A3 --> A4{Right place?}
        A4 -- no --> A2
        A4 -- not sure --> A3
        A4 -- yes --> A5[[Finally edit]]
    end
    subgraph withwiki [With a page wiki - look up the index]
        direction TB
        B1([Prompt]) --> B2[Resolve page via wiki-index]
        B2 --> B3[Read one page-wiki.md]
        B3 --> B4[[Edit the right files]]
    end
{{< /mermaid >}}
...
Resolve the page, check it's fresh, reuse the page. The source never gets opened.
